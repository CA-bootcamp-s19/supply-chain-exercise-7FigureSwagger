...
contract SupplyChain {
	...
	*/
    ...
    modifier forSale(uint256 _sku) {
        require(
            items[_sku].state == 0 && items[_sku].price != 0,
            "Item is not for sale!"
        );
        _;
    }
    modifier sold(uint256 _sku) {
        require(items[_sku].state == 1, "Item is already sold!");
        _;
    }
    modifier shipped(uint256 _sku) {
        require(items[_sku].state == 2, "Item has yet to be shipped!");
        _;
    }
    modifier received(uint256 _sku) {
        require(items[_sku].state == 3, "Item has yet to be received!");
        _;
    }
    ...
    {
        emit LogSold(sku);
        address payable x = items[sku].seller;
        if (msg.value >= items[sku].price) x.transfer(items[sku].price);
